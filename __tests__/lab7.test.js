describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    // page.$(...) means "find the first matching element"
    const productItem = await page.$('product-item');
    // get shadow root and find button within shadow root
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    const innerTextButton = await cartCount.getProperty('innerText');
    const cartCountText = await innerTextButton.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      ...
      expect(cartText).toBe('Add to Cart');
    }
    const cartCount = await page.$('#cart-count');
    const innerText = await cartCount.getProperty('innerText');
    const cartCountText = await innerText.jsonValue();
